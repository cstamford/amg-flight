...
public class PlayerControllerLouis : MonoBehaviour
{
	...
	private FlightControllerLouis m_flight;
	...
	void Start ()
	{
		...
		m_flight = this.GetComponent ("FlightController") as FlightControllerLouis;
	}
	...
	void Update()
	{
		...
		if (m_isFlying)
		{
			...
			transform.position = m_flight.getPosition();
			transform.eulerAngles = m_flight.getRotation();
		}
		else
		{
	...
	private void HandleFlying()
	{
		...
		//m_isFlying = Physics.Raycast (transform.position, -Vector3.up, m_colliderHeight + m_playerHeight + 0.1f));
	}
