...
namespace Marc
{
    public class GroundController : ControllerBase, IControllerBase
    {
		...
		private readonly AudioClip[] m_walkSoundClips;
            ...
            : base(controller)
        {
			...
			m_walkSoundClips = new AudioClip[15];
			for (int i=0; i<15; i++) 
			{
				int j = i+1;
				m_walkSoundClips[i] = (AudioClip)Resources.Load("grass walk (" + j + ")");
			}
        ...
        private void handleAudio()
        {
            if (!m_falling)
            {
				if (m_walking)
                {
					...
					{
						m_walkSound.clip = m_walkSoundClips[(int)(Random.Range(0, 14))];
						Debug.Log(m_walkSound.clip);
						m_walkSound.Play();
						m_walkSoundPlaying = true;
					}
                }
